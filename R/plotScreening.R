...
# Put this file under R/ (e.g., R/plot-screening.R)
# -------------------------------------------------------------------------
...
.pp_get_measure <- function(x, measure = NULL, coef = 1L) {
  ...
  # where coef=1 corresponds to alpha; adapt as needed for your object.
...
bucketize_screening <- function(x, nbucket = 50L, measure = NULL, annualize = 1,
  ...
  out <- lapply(seq_len(nbucket), function(b) {
    ...
    # numerical safety: clamp + renormalize
...
#' @export
#' @noRd
# ---- the plot method (internal worker) ----
plot.pp_screening <- function(x, nbucket = 50L,
  ...
  y <- B$bucket  # bucket 1 = best (bottom)
  plot(B$metric, y,
       pch = 16, cex = 0.7,
       xlab = "", ylab = "",
       yaxt = "n", main = measure,
  ...
  # ---- Right: stacked bars per bucket (paper-like geometry) ----
  nb <- nrow(B)
  
  plot(NA,
       xlim = c(0, 1), ylim = c(0, 1),
       xaxs = "i", yaxs = "i",     # IMPORTANT: no internal padding
       xlab = "", ylab = "",
       axes = FALSE, main = "")
  ...
  for (b in seq_len(nb)) {
    ...
    x2 <- x1 + sort(B$pizero, decreasing = TRUE)[b]#B$pizero[b]
  ...
  # axis and label (put label on TOP like the paper)
  ...
  if (show_diag) {
    for (cst in c(2/3, 1, 4/3)) {  # nice spacing like the paper
